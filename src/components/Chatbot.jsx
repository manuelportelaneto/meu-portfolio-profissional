...
const Chatbot = () => {
  ...
  useEffect(() => {
    ...
    if (isOpen && messages.length === 0 && sessionId) {
      setIsLoading(true);
      fetch('https://manuel-bot-backend-153290184725.southamerica-east1.run.app/api/chat', {
        method: 'POST',
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await fetch('https://manuel-bot-backend-153290184725.southamerica-east1.run.app/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
